...
select * from salesorder order by order_date ASC limit 5;
#25. Display next ten sales order information from sales order table except first five sales order.
select * from salesorder order by order_date ASC limit 10 offset 5;
...
group by province
having totalclients > 1
order by totalclients DESC;
#27. Display information clients have number of sales order more than 1.
select * from clients where client_number in (select client_number from salesorder
...
select count(product_name) from product where product_name = 'TV';
...
#(in ascending order).
select p.product_number, product_name, count(*) numberOfOrder
from product p inner join salesorderdetails s on s.product_number = p.product_number
...
select * from product;
select * from product where quantity_on_hand > 20 and quantity_on_hand 
